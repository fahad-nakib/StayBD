...
export default function AdminServicesPage() {
  ...
  // ── FIX 1: Pass { limit: 200 } to getServices so all items are fetched ──
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [servicesRes, experiencesRes] = await Promise.all([
        adminAPI
          .getServices({ limit: 200 })
          .catch(() => ({ data: { data: [] } })),
        adminAPI.getAdminExperiences().catch(() => ({ data: { data: [] } })),
      ]);
      ...
      const sData =
        ...
        servicesRes.data ||
        [];
      const safeServices = Array.isArray(sData)
        ? sData.map((s) => ({ ...s, itemType: "service" }))
        : [];

      // Process Experiences
      const eData =
        experiencesRes.data?.data?.experiences ||
        experiencesRes.data?.data ||
        experiencesRes.data ||
        [];
      const safeExperiences = Array.isArray(eData)
        ? eData.map((e) => ({ ...e, itemType: "experience" }))
        : [];

      // Combine and sort by newest first
      const combined = [...safeServices, ...safeExperiences].sort(
  ...
  // ── FIX 2: Also show approved items with hasPendingChanges under "pending" tab ──
  const filteredItems = items.filter((item) => {
    // If viewing the deletion-requests tab, ONLY show items requesting deletion
    if (status === "deletion-requests") return item.deletionRequested === true;

    // Prevent deletion requests from leaking into pending/approved tabs
    if (item.deletionRequested) return false;

    // Approved services/experiences with pending changes belong in the pending tab
    if (status === "pending" && item.hasPendingChanges) return true;

    // Finally, match standard statuses
  ...
  const approve = async (item, isUpdate = false) => {
    try {
      const payload = { action: "approve", status: "approved" };
      if (item.itemType === "service") {
        await adminAPI.approveService(item._id, payload);
      } else {
        await adminAPI.approveExperience(item._id, payload);
      }
      toast.success(
        isUpdate ? "Changes applied!" : `${item.itemType} approved!`,
      );
      loadData();
    } catch (err) {
      toast.error(`Failed to approve ${item.itemType}`);
    }
  };
  ...
  const reject = async () => {
    ...
    try {
      const payload = { action: "reject", status: "rejected", reason };
      if (item.itemType === "service") {
        await adminAPI.approveService(item._id, payload);
      } else {
        await adminAPI.approveExperience(item._id, payload);
      ...
      toast.success(
        isUpdate ? "Changes rejected" : `${item.itemType} rejected`,
      );
      closeRejectModal();
      loadData();
    } catch (err) {
      toast.error(`Failed to reject ${item.itemType}`);
    }
  };

  // 👇 Handle Provider's Deletion Request 👇
  const handleDeletion = async (item, action) => {
    try {
      const payload = { action }; // 'approve' or 'reject'
      if (item.itemType === "service") {
        await adminAPI.handleServiceDeletionRequest(item._id, payload);
      } else {
        await adminAPI.handleExperienceDeletionRequest(item._id, payload);
      }
  ...
  const handleDirectDelete = async (item) => {
    if (
      !window.confirm(
        `Are you sure you want to permanently delete this ${item.itemType}? This cannot be undone.`,
      )
    )
      return;
    ...
    try {
      if (item.itemType === "service") {
        await adminAPI.deleteService(item._id);
      } else {
        await adminAPI.deleteExperience(item._id);
      }
      toast.success(
        `${item.itemType.charAt(0).toUpperCase() + item.itemType.slice(1)} permanently deleted`,
      );
      loadData();
    } catch (err) {
      toast.error(`Failed to directly delete ${item.itemType}`);
    }
  };
        ...
        >
          {items.length === 0 && loading ? (
            ...
            <div className="text-center py-16 text-gray-400">
              <p>No {status.replace("-", " ")} services or experiences</p>
            </div>
          ) : (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-14 h-12 rounded-lg bg-gray-100 overflow-hidden shrink-0 border shadow-sm flex items-center justify-center relative">
                            {img ? (
                              ...
                              <FiTool className="text-gray-300" size={20} />
                            ) : (
                              <FiCalendar className="text-gray-300" size={20} />
                            )}
                            {/* Small Type Indicator */}
                            <div
                              className={`absolute bottom-0 left-0 right-0 h-1 ${item.itemType === "service" ? "bg-orange-500" : "bg-purple-500"}`}
                            />
                          </div>
                          <div>
                            <p
                            ...
                            <div className="flex gap-1 mt-1 flex-wrap">
                              <span
                                className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider text-white ${item.itemType === "service" ? "bg-orange-500" : "bg-purple-500"}`}
                              >
                                {item.itemType}
                              </span>
                      ...
                      <td className="py-4 px-4">
                        <p className="font-medium text-gray-800">
                          {item.provider?.name || item.host?.name || "Unknown"}
                        </p>
                      ...
                      <td className="py-4 px-4 text-gray-600 text-xs">
                        {item.itemType === "service"
                          ? [
                              ...(item.serviceArea?.areas || []),
                              ...(item.serviceArea?.districts || []),
                            ]
                              .filter(Boolean)
                              .join(", ") || "N/A"
                      ...
                      <td className="py-4 px-4 font-bold text-gray-900">
                        {item.itemType === "service"
                          ? `${formatCurrency(item.pricePerHour ?? item.hourlyRate ?? 0)}/hr`
                          : `${formatCurrency(item.pricePerPerson ?? 0)}/pp`}
                      </td>
                      <td className="py-4 px-4">
            ...
            <div className="p-6 border-b flex justify-between items-center bg-gray-50">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-wider text-white ${reviewModal.item.itemType === "service" ? "bg-orange-500" : "bg-purple-500"}`}
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-3 bg-gray-50 rounded-xl border">
                        ...
                        <p className="font-bold text-lg text-gray-900">
                          {reviewModal.item.itemType === "service"
                            ? `${formatCurrency(reviewModal.item.pricePerHour ?? reviewModal.item.hourlyRate ?? 0)}/hr`
                            : `${formatCurrency(reviewModal.item.pricePerPerson ?? 0)}/pp`}
                        </p>
                      </div>
                      <div className="p-3 bg-gray-50 rounded-xl border">
                        <label className="text-[10px] font-bold text-gray-400 uppercase">
                        ...
                        <p className="font-bold text-gray-700 capitalize line-clamp-2">
                          {SERVICE_CATEGORIES.find(
                            (c) => c.value === reviewModal.item.category,
                          )?.label ||
                            reviewModal.item.category ||
                            "Experience"}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-6 bg-yellow-50 p-6 rounded-2xl border border-yellow-200 shadow-inner">
                    ...
                    <div className="space-y-4">
                      {Object.entries(
                        ...
                        // ── FIX 3: Use explicit null/undefined check instead of
                        // !value which hides valid fields like availability ──
                        if (
                          key === "_id" ||
                          value === null ||
                          value === undefined ||
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="p-4 bg-primary-50 rounded-2xl border border-primary-100 flex items-center gap-4">
                      <div className="w-14 h-14 rounded-full bg-white flex items-center justify-center font-bold text-primary-600 text-xl shadow-sm border border-primary-100">
                        {(
                          reviewModal.item.provider?.name ||
                          reviewModal.item.host?.name ||
                          "?"
                        ).charAt(0)}
                      </div>
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {reviewModal.item.provider?.email ||
                    ...
                    <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                      ...
                      <p className="text-sm text-blue-900 font-medium">
                        {reviewModal.item.itemType === "service"
                          ? [
                              ...(reviewModal.item.serviceArea?.areas || []),
                              ...(reviewModal.item.serviceArea?.districts ||
                                []),
                            ]
                              .filter(Boolean)
                    ...
                    {reviewModal.item.deletionRequested && (
                      <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-xl">
                        ...
                        <p className="text-xs text-red-600">
                          The provider has requested to permanently remove this
